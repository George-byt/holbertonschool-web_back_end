...
| `Promise.prototype.then()` | The `then()` method returns a Promise. It takes up to two arguments: callback functions for the success and failure cases of the Promise. | ```p.then(onFulfilled[, onRejected]); p.then(value => // fulfillment }, reason => { // rejection });```|